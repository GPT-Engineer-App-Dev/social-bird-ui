...
const Index = () => {
  ...
  const handleReply = (id) => {
    setReplies({ ...replies, [id]: !replies[id] });
  };
  ...
  const Tweet = ({ tweet, depth = 0 }) => (
    <Box key={tweet.id} p={4} borderBottom="1px" borderColor="gray.200" ml={depth * 4}>
      <HStack spacing={4}>
        <Avatar name="User" />
      ...
      {tweets.filter(t => t.parentId === tweet.id).map(reply => (
        <Tweet key={reply.id} tweet={reply} depth={depth + 1} />
      ))}
    </Box>
  );
        ...
        >
          ...
          <HStack spacing={4} p={4} borderBottom="1px" borderColor="gray.200">
            ...
            <Textarea 
              ...
              onChange={(e) => setTweetContent(e.target.value)} 
            />
            <Button onClick={() => handleTweet(null)} colorScheme="blue">Tweet</Button>
          </HStack>
